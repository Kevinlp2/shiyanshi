...
<script type="text/javascript" src="/shiyanshi/admin/js/Date.js"></script>
</head>
<body>
    ...
    <table class="table table-hover text-center">
       ...
       <s:iterator value="page.pageList" >
        <tr>
         ...
         <%--  <s:if test="pj==null"><td>还没有评论</td></s:if><s:else>
          ...
          <s:if test="ostate=='已支付'"> <a class="button border-main" href="orderfahuo?order.oid=<s:property value="oid" />"><span class="icon-edit"></span> 未发货</a> </s:if>
      ...
      <s:if test="#session.p!=2">
       <td colspan="2">当前显示第
			...
			<td colspan="4">
			<div align="center">
				<a href="orderlist.action?page.pageIndex=1"> 首页</a>
				<s:property value="totalpages" />
				<a <s:if test="page.pageIndex<=1"> href="javascript:false;"</s:if>
					<s:else>href="orderlist.action?page.pageIndex=<s:property value='page.pageIndex-1' />"</s:else>>上一页</a>
				<a
					<s:if test="page.pageIndex>=page.totalpages" > href="javascript:false;"</s:if>
					<s:else>href="orderlist.action?page.pageIndex=<s:property value='page.pageIndex+1' />"</s:else>>下一页</a>
				<a
					href="orderlist.action?page.pageIndex=<s:property value="page.totalpages" />">末页</a>
			...
			<td colspan="4">
			<div align="center">
				<a href="orderquery.action?page.pageIndex=1"> 首页</a>
				<s:property value="totalpages" />
				<a <s:if test="page.pageIndex<=1"> href="javascript:false;"</s:if>
					<s:else>href="orderquery.action?page.pageIndex=<s:property value='page.pageIndex-1' />"</s:else>>上一页</a>
				<a
					<s:if test="page.pageIndex>=page.totalpages" > href="javascript:false;"</s:if>
					<s:else>href="orderquery.action?page.pageIndex=<s:property value='page.pageIndex+1' />"</s:else>>下一页</a>
				<a
					href="orderquery.action?page.pageIndex=<s:property value="page.totalpages" />">末页</a>
			</div></td> 
			</s:else>     
      </tr>
    </table>
