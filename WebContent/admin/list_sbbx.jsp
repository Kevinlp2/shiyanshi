        ...
        <table class="table table-hover text-center">
            ...
            <c:forEach items="${list }" var="yp">
                <tr>
                    ...
                    <td><c:forEach items="${slist }" var="hs1">
                        <c:if test="${yp.sid==hs1.id }">
                            ${hs1.name }
                        </c:if>
                    </c:forEach>
                    </td>
                    ...
                    <td>
                        <div class="button-group">
                            <c:if test="${sessionScope.auser.utype=='实验设备管理员' and yp.status=='待审核' }">
                                <a class="button border-main" href="tongGuoShenBei.do?id=${yp.kid }"><span
                                        class="icon-edit"></span>通过审核</a>&nbsp;&nbsp;
                                <a class="button border-red" href="buTongGuoShenBei.do?id=${yp.kid }"
                                   onclick="return del(1,1,1)"><span class="icon-trash-o"></span>不通过</a>
                            </c:if>
                        </div>
                    </td>
                </tr>
